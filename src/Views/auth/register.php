...
.register-card{background:var(--c1);border:1px solid var(--border2);border-radius:var(--r);padding:36px 32px;position:relative;overflow:hidden;animation:riseUp .5s ease both}
      ...
      <div class="form-group">
        ...
        <select name="id_rol" class="form-control" required>
          ...
          <?php foreach ($roles as $rol): ?>
            <option value="<?= $rol->id ?>" <?= (isset($_POST['id_rol']) && $_POST['id_rol'] == $rol->id) ? 'selected' : '' ?>>
...
(function frame(){c.clearRect(0,0,W,H);for(let i=0;i<pts.length;i++){for(let j=i+1;j<pts.length;j++){const d=Math.hypot(pts[i].x-pts[j].x,pts[i].y-pts[j].y);if(d<100){c.beginPath();c.moveTo(pts[i].x,pts[i].y);c.lineTo(pts[j].x,pts[j].y);c.strokeStyle=`rgba(0,212,255,${.05*(1-d/100)})`;c.lineWidth=.5;c.stroke()}}}pts.forEach(p=>{p.tick();p.draw()});requestAnimationFrame(frame);})();
</script>
</body>
</html>
