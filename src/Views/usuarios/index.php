...
<title>EventCore · Usuarios</title>
...
.role-tag{padding:3px 10px;border-radius:6px;font-size:10px;text-transform:uppercase;font-weight:600;letter-spacing:0.5px;background:var(--cyan-g);color:var(--cyan)}
...
<div class="shell">
  ...
  <div class="main">
    <header class="tb"><div class="tb-title">Gestión de Usuarios</div></header>
    <div class="con">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:30px">
        <h2 style="font-weight:200;font-size:24px">Administradores <strong style="color:var(--cyan)">Vigentes</strong></h2>
        <a href="<?= BASE_URL ?>/usuarios?action=crear" class="btn btn-p">+ Nuevo Administrador</a>
      </div>
      ...
      <div class="card">
        <table class="et">
          ...
          <tbody>
            ...
            <tr>
              <td>
                <div style="font-weight:500;color:var(--t1)"><?= $e($u['nombres'] . ' ' . $u['apellidos']) ?></div>
              </td>
              <td style="font-family:'JetBrains Mono';font-size:12px;color:var(--t2)"><?= $e($u['email']) ?></td>
              <td><span class="role-tag"><?= $e($u['rol']) ?></span></td>
              <td>
                <?php if($u['activo']): ?>
                  <span style="color:var(--lime);font-size:11px">● Activo</span>
                <?php else: ?>
                  <span style="color:var(--rose);font-size:11px">● Inactivo</span>
                <?php endif; ?>
              </td>
              <td><div style="font-family:'JetBrains Mono';font-size:11px"><?= date('d/m/Y', strtotime($u['creado_en'])) ?></div></td>
              <td style="text-align:right">
                <div style="display:flex;justify-content:flex-end;gap:12px">
                  <a href="<?= BASE_URL ?>/usuarios?action=editar&id=<?= $u['id_usuario'] ?>" style="color:var(--cyan);text-decoration:none;font-size:12px;font-weight:500">Editar</a>
                  <?php if($u['activo']): ?>
                    <a href="<?= BASE_URL ?>/usuarios?action=eliminar&id=<?= $u['id_usuario'] ?>" onclick="return confirm('¿Desactivar este usuario?')" style="color:var(--rose);text-decoration:none;font-size:12px;font-weight:500">Desactivar</a>
                  <?php endif; ?>
                </div>
              </td>
            </tr>
