...
<title>EventCore · Crear Usuario</title>
...
.card{background:var(--c1);border:1px solid var(--border);border-radius:var(--r);padding:40px;max-width:700px;margin:0 auto;box-shadow:0 20px 50px rgba(0,0,0,0.3);position:relative}
  ...
  <div class="main">
    <div class="con">
      <div style="margin-bottom:30px;display:flex;justify-content:space-between;align-items:center">
        <h2 style="font-weight:200;font-size:28px">Nuevo <strong>Administrador</strong></h2>
        <a href="<?= BASE_URL ?>/usuarios" class="btn btn-g">← Volver</a>
      </div>
        ...
        <form method="POST">
          <div class="form-row">
            <div class="form-group">
              <label>Nombres</label>
              <input type="text" name="nombres" class="form-control" required value="<?= $e($_POST['nombres'] ?? '') ?>" placeholder="Ej. Juan">
            </div>
            <div class="form-group">
              <label>Apellidos</label>
              <input type="text" name="apellidos" class="form-control" value="<?= $e($_POST['apellidos'] ?? '') ?>" placeholder="Ej. Pérez">
            </div>
          </div>
          <div class="form-group">
            <label>Correo Electrónico</label>
            <input type="email" name="email" class="form-control" required value="<?= $e($_POST['email'] ?? '') ?>" placeholder="juan.perez@eventcore.com">
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Contraseña</label>
              <input type="password" name="password" class="form-control" required placeholder="Min. 5 caracteres">
            </div>
            <div class="form-group">
              ...
              <select name="id_rol" class="form-control" required>
                ...
                <option value="<?= $r['id_rol'] ?>" <?= (($_POST['id_rol'] ?? '') == $r['id_rol']) ? 'selected' : '' ?>><?= $e($r['nombre_rol']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          <div style="margin-top:30px;display:flex;gap:15px">
            ...
            <a href="<?= BASE_URL ?>/usuarios" class="btn btn-g">Cancelar</a>
          </div>
        </form>
      </div>
    </div>
