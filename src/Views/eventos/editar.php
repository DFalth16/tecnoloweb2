...
<title>EventCore · Editar Evento</title>
  ...
  <div class="main">
    <div class="con">
      <div style="margin-bottom:30px;display:flex;justify-content:space-between;align-items:center">
        <h2 style="font-weight:200;font-size:28px">Editar <strong>Evento</strong></h2>
        <a href="<?= BASE_URL ?>/eventos" class="btn btn-g">← Cancelar Edición</a>
      </div>
      ...
      <div class="card">
        <form method="POST">
          <div class="form-group">
            ...
            <input type="text" name="titulo" class="form-control" required value="<?= $e($evento['titulo']) ?>">
          </div>
          <div class="form-group">
            <label>Descripción</label>
            <textarea name="descripcion" class="form-control" rows="4"><?= $e($evento['descripcion']) ?></textarea>
          </div>
          <div class="form-row">
            <div class="form-group">
              ...
              <select name="id_categoria" class="form-control" required>
                ...
                <option value="<?= $c['id_categoria'] ?>" <?= ($evento['id_categoria'] == $c['id_categoria']) ? 'selected' : '' ?>><?= $e($c['nombre']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="form-group">
              ...
              <select name="id_sede" class="form-control" required>
                ...
                <option value="<?= $s['id_sede'] ?>" <?= ($evento['id_sede'] == $s['id_sede']) ? 'selected' : '' ?>><?= $e($s['nombre']) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              ...
              <input type="datetime-local" name="fecha_inicio" class="form-control" required value="<?= date('Y-m-d\TH:i', strtotime($evento['fecha_inicio'])) ?>">
            </div>
            <div class="form-group">
              <label>Fecha Fin</label>
              <input type="datetime-local" name="fecha_fin" class="form-control" required value="<?= date('Y-m-d\TH:i', strtotime($evento['fecha_fin'])) ?>">
            </div>
          </div>
          <div class="form-row">
            <div class="form-group">
              <label>Cupo Máximo</label>
              <input type="number" name="cupo_maximo" class="form-control" required min="1" value="<?= $e($evento['cupo_maximo']) ?>">
            </div>
            <div class="form-group">
              <label>Precio Entrada (Bs.)</label>
              <input type="number" name="precio_entrada" class="form-control" step="0.01" min="0" value="<?= $e($evento['precio_entrada']) ?>" id="precioInput" <?= $evento['es_gratuito'] ? 'disabled' : '' ?>>
            </div>
          </div>
          <div class="form-group">
            <label style="display:flex;align-items:center;gap:10px;text-transform:none">
              <input type="checkbox" name="es_gratuito" onchange="document.getElementById('precioInput').disabled=this.checked; if(this.checked) document.getElementById('precioInput').value='0';" <?= $evento['es_gratuito'] ? 'checked' : '' ?>>
              Evento Gratuito
            </label>
          </div>
          <div style="margin-top:30px;display:flex;gap:15px">
            <button type="submit" class="btn btn-p" style="flex:1">Guardar Cambios</button>
            <a href="<?= BASE_URL ?>/eventos" class="btn btn-g">Descartar</a>
          </div>
        </form>
      </div>
    </div>
