...
.obar{width:100px;height:6px;background:rgba(255,255,255,.05);border-radius:3px;display:inline-block;overflow:hidden;vertical-align:middle;margin-right:8px}
.ofill{height:100%;display:block;transition:width 1s var(--ease);box-shadow:0 0 10px rgba(0,212,255,0.3)}
...
.form-control{background:var(--c2);border:1px solid var(--border);color:var(--t1);padding:12px 16px;border-radius:10px;outline:none;transition:all .3s var(--ease);font-family:inherit}
...
<div class="shell">
  ...
  <div class="main">
    <header class="tb"><div style="flex:1;font-size:15px">Gestión de Eventos</div></header>
    <div class="con">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <h2 style="font-weight:200">Listado de <strong>Eventos</strong></h2>
        <a href="<?= BASE_URL ?>/eventos?action=crear" class="btn btn-p">+ Nuevo Evento</a>
      </div>

      <div class="card" style="margin-bottom:20px">
        <form method="GET" style="display:flex;gap:10px">
          <input type="hidden" name="action" value="index">
          <input type="text" name="q" class="form-control" placeholder="Buscar..." value="<?= $e($search) ?>" style="flex:1">
      ...
      <div class="card">
        <table class="et">
          <thead><tr><th>Evento</th><th>Sede</th><th>Fecha</th><th>Ocupación</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
            ...
            <tr>
              <td>
                <div style="color:var(--t1)"><?= $e($ev['titulo']) ?></div>
                <div style="font-size:11px;color:var(--t4)"><?= $e($ev['categoria']) ?> · <?= $e($ev['codigo_evento']) ?></div>
              </td>
              <td><?= $e($ev['sede']) ?></td>
              <td><?= date('d/m/Y', strtotime($ev['fecha_inicio'])) ?></td>
              <td>
